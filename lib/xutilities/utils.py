...
def get_usable_gpu(threshold=2048, req_cards=1, gpu_id_map=None):
    """Find a usable gpu
    ...
    try:
        ...
        id_mem_sorted = sorted(free_id_mem, key=lambda x: x[1])
        
        bests = id_mem_sorted[-req_cards:]
        if bests[-req_cards][1] < threshold:
            gpu_id = None
        else:
        ...
        logging.getLogger(myself()).warning(e)

    return gpu_id


def wait_gpu(req_mem=8000, req_cards=1, id_map=None):
    wait_time = int(random.random() * 5)
    time.sleep(wait_time)
    lock_gpu()
    while True:
        gpu_id = get_usable_gpu(req_mem, req_cards=req_cards, gpu_id_map=id_map)
    ...
    for n, p in named_parameters:
        if p.requires_grad:
            if p.grad is not None:
                ...
                n_ave_grads.update({n: -1})
                n_max_grads.update({n: -1})
    ...
    for n, p in model.named_parameters():
        if p.requires_grad:
            ...
            if p.grad is not None:
                ...
                print(f'layer: {n}, shape: {list(p.shape)}, ave grad: {ave_grads[-1]:.8f}, max grad: {max_grads[-1]:.8f}.')
            else:
    ...
    def step(self, loss=None, acc=None, criterion=lambda x1, x2: x1 or x2):
        ...
        if criterion(better_loss, better_acc):
            logging.getLogger(myself()).info(
                ...
                f'highest acc: {acc:.4}, lowest loss: {loss:.4}.')
...
class RunningAverage:
    ...
    def value(self):
        try:
            return self.data.sum() / min(self.step, self.window_size)
...
def init(parse_list=None, user_param=None, user_args_modifier=None, pytorch_deterministic=True):
    ...
    if not args.debug_mode:
        _uuid = args.uuid if args.uuid is not None else str(uuid.uuid4().hex)[:8]
        args.model_id = f'{_uuid}_{hostname}_{gpu_id}_{os.getpid()}_{git_branch}_{git_commit}_{date_time}'
    else:
